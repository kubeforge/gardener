...
func ValidateCloudProfileSpec(spec *garden.CloudProfileSpec, fldPath *field.Path) field.ErrorList {
	...
	if _, err := helper.DetermineCloudProviderInProfile(*spec); err != nil {
		allErrs = append(allErrs, field.Forbidden(fldPath.Child("aws/azure/gcp/alicloud/openstack/kubevirt/local"), "cloud profile must only contain exactly one field of aws/azure/gcp/alicloud/openstack/kubevirt/local"))
		return allErrs
	}
...
func ValidateShootSpec(spec *garden.ShootSpec, fldPath *field.Path) field.ErrorList {
	...
	if _, err := helper.DetermineCloudProviderInShoot(spec.Cloud); err != nil {
		allErrs = append(allErrs, field.Forbidden(cloudPath.Child("aws/azure/gcp/alicloud/openstack/kubevirt/local"), "cloud section must only contain exactly one field of aws/azure/gcp/alicloud/openstack/kubevirt/local"))
		return allErrs
	}
